...
図\ref{fig:performance_curves}は，
横軸にポール質量のデフォルト値からの倍率，
縦軸に IQM（Interquartile Mean）を示している．
緑色の帯状領域は訓練環境において設定された質量のランダム化範囲（0.5--2.5倍）を表す．
それ以外の範囲は訓練環境で設定されていない質量で, 境界を赤色の点線でOOD Boundaryとして示している．
各手法の性能曲線は，赤（Oracle TD-MPC2），黄色（Domain Randomization），緑（提案手法：Adaptive），青（Non-adaptive TD-MPC2）で示されており，
各曲線周辺の淡色の帯は標準偏差を表す．
評価は5つの独立した乱数シードごとに，それぞれ30エピソード分の累積報酬を収集して算出している．

\begin{figure}[tb]
\centering
\includegraphics[width=0.85\linewidth]{figures/pendulum_estimation_convergence2.png}
...
\begin{table}[tb]
    ...
    \begin{tabular}{lcccc}
    \toprule
    \textbf{質量(倍率)} & \textbf{Non-adaptive} & \textbf{DR} & \textbf{Adaptive (Ours)} & \textbf{Oracle} \\
    \midrule
...
図\ref{fig:walker_actuator_performance}は，図\ref{fig:performance_curves}と同様に，
横軸にアクチュエータ出力スケールのデフォルト値からの倍率，
縦軸に IQMを示している．
緑色の帯状領域は訓練環境において設定されたアクチュエータ出力スケールのランダム化範囲（0.4--1.4倍）を表す．
その範囲外は訓練環境で設定されていないアクチュエータ出力スケールで, 境界を赤色の点線でOOD Boundaryとして示している．
各手法の性能曲線は，赤（Oracle TD-MPC2），黄色（Domain Randomization），緑（提案手法：Adaptive），青（Non-adaptive TD-MPC2）で示されており，
各曲線周辺の淡色の帯は標準偏差を表す．
評価は5つの独立した乱数シードごとに，それぞれ30エピソード分の累積報酬を収集して算出している．
...
\begin{figure}[tb]
\centering
\includegraphics[width=0.9\linewidth]{figures/walker_actuator_performance_curves2.png}
...
上段のグラフは, 横軸に時刻，縦軸にアクチュエータ出力スケールを示しており,
下段のグラフは, 横軸に時刻, 縦軸に単一ステップあたりの報酬を示している.
赤色の線はOracle TD-MPC2，黄色の線はDomain Randomization，緑色の線は提案手法：Adaptive，青色の線はNon-adaptive TD-MPC2を示しており,
各曲線周辺の淡色の帯は標準偏差を表す．評価は5つの独立した乱数シードごとに，それぞれ30エピソード分の累積報酬を収集して算出している．

\begin{figure}[tb]
\centering
\includegraphics[width=0.85\linewidth]{figures/shuusei.png}
...
左上, 右上, 左下, 右下のグラフはそれぞれ, 真値質量が0.6倍, 1.0倍, 1.4倍, 1.8倍の条件において,
横軸に時刻, 縦軸に推定質量のデフォルト値からの倍率を示している．評価は一つのシードを用いて行った.
...
左上, 右上, 左下, 右下のグラフはそれぞれ, 真値スケールが0.6倍, 0.8倍, 1.0倍, 1.2倍の条件において,
横軸に時刻, 縦軸に推定スケールを示している．評価は一つのシードを用いて行った.
...
\subsection{Walker-Walk におけるアクチュエータスケール追従挙動}
...
真のスケール変化と推定値の時系列を示す．
図上の赤色の点線は各時刻における真のスケールを, 青色の線は推定されたスケール, 青色のシェードは推定値の標準偏差を示している．

\begin{figure}[tb]
\centering
\includegraphics[width=0.9\linewidth]{figures/walker_dynamic_estimation_timeseries.png}
\caption{Walker-Walk における動的環境でのアクチュエータスケール推定挙動}
