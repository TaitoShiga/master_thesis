...
そのモデル上で将来の状態および報酬を予測する枠組みである\cite{ha2018world}．
モデルベース強化学習では，
...
それぞれ学習された報酬モデルおよび価値関数によって予測される．
潜在状態の推移は，
...
TD 学習による価値関数推定を統合したモデルベース強化学習手法であり,
...
これらのモデルは，
観測系列 $\{\mathbf{o}_t, \mathbf{o}_{t+1}, \ldots\}$ および
行動系列 $\{\mathbf{a}_t, \mathbf{a}_{t+1}, \ldots\}$ から得られる遷移データを用いて，
多段階の潜在ロールアウトに基づく損失関数を最小化することで同時に学習される．
...
行動選択時には，
\ref{subsec:latent_mpc} で定義した MPC が用いられ，
...
\subsection{履歴情報に基づくコンテキスト抽出と GRU}
\label{subsec:gru}
...
という情報が蓄積されており，
未知の環境特性を同定するための十分な統計量が含まれると考えられる．

履歴から動的なコンテキストを抽出する写像を
\(
...
履歴表現 $\mathbf{c}_t$ を抽出する\cite{dey2017gate}．
GRU は更新ゲートおよびリセットゲートによって
情報の保持と忘却を動的に制御する構造を持つ．
この機構により，
過去の相互作用の中に潜むダイナミクスの徴候を効率的に集約し，
未知の物理パラメータに適応するための
状態表現を獲得することが可能となる．
